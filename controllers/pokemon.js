...
exports.getData = async (req, res) => {
  try {
    ...
    if (Number.isNaN(page) || page < 1) {
      return res
        .status(400)
        .send({ error: 'Invalid pagination: page must be a positive number' });
    }
    if (Number.isNaN(limit) || limit < 1) {
      return res
        .status(400)
        .send({ error: 'Invalid pagination: limit must be a positive number' });
    }
    ...
    if (types) {
      filter.types = {
        $in: types.split(',').map((type) => type.toLowerCase()),
      };
    }
    if (abilities) {
      filter.abilities = {
        $in: abilities.split(',').map((ability) => ability.toLowerCase()),
      };
    }

    // Obtener los datos de la base de datos con los filtros
    const data = await pokemonModel
      .find(filter)
      .skip(skip)
      .limit(pageLimit)
...
exports.insertData = async (req, res) => {
  try {
    const { name, img, thumbnailUrl, largeImageUrl, types, abilities, stats } =
      req.body;
    if (!stats || Object.values(stats).some((stat) => stat <= 0)) {
      return res
        .status(400)
        .send({ error: 'All stats must be greater than 0' });
    }
    const existingPokemon = await pokemonModel.findOne({ name });
    if (existingPokemon) {
      return res
        .status(400)
        .send({ error: 'A Pokémon with this name already exists' });
    }
...
exports.deleteData = async (req, res) => {
  try {
    ...
    if (result.deletedCount === 0) {
      return res
        .status(404)
        .send({ error: 'the pokemon was not found to delete' });
    }
